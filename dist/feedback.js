...
export async function recordRecallFeedback(store, config, input) {
    const recall = await store.get(input.recallId.trim());
    if (!recall || recall.activity?.type !== "recall")
        throw new Error(`Recall activity not found: ${input.recallId}`);
    ...
    if (overlap)
        throw new Error(`Memory cannot be both relevant and irrelevant: ${overlap}`);
    const recalledIds = new Set(recall.activity.memoryIds);
    for (const id of [...relevantIds, ...irrelevantIds]) {
        if (!recalledIds.has(id))
            throw new Error(`Memory ${id} was not part of recall ${recall.id}`);
    }
    const at = new Date().toISOString();
    const id = randomUUID();
    ...
    await store.add({
        ...
        tags: normalizeTags([
            ...
            "role-tool",
            ...recall.tags.filter((tag) => tag !== "pathmark-activity" &&
...
function commonScopeTags(results) {
    if (results.length === 0)
        return [];
    const scopes = results.map((result) => new Set(result.record.tags.filter((tag) => /^(?:namespace|workspace|project):/.test(tag))));
