...
export async function runMcpServer() {
    ...
    server.registerTool("create_conclusion", {
        ...
        if (decision && !evidenceIds?.length)
            throw new Error("A decision requires supporting evidenceIds");
    ...
    }, async ({ id, decidedBy, note, expectedRevision }) => jsonText({ rejected: (await store.decideConclusion(id, "rejected", { decidedBy, note, expectedRevision })) ?? null }));
...
async function validatedEvidenceIds(store, ids, conclusionTags) {
    ...
    const scopeTags = conclusionTags.filter((tag) => ["namespace:", "workspace:", "project:", "project-id:", "session:"].some((prefix) => tag.startsWith(prefix)));
