...
async function proactivePromptContext(store, input, promptText, options) {
...
function memoryBlock(results, memoryFile, options = {}) {
    return [
        "<pathmark-memory>",
...
function visibleRecallInstruction(input) {
    const args = {
        query: input.query,
        limit: input.limit,
        ids: input.ids,
...
function promptRecallTagFilters(input) {
    const filters = [];
    const workspaceTag = workspaceTagFromCwd(input.cwd);
    if (workspaceTag)
...
function isCurrentImmediatePrompt(result, input, promptText) {
    ...
    if (!record.tags.includes(IMMEDIATE_PROMPT_TAG))
        return false;
    if (record.source.toLowerCase() !== `codex:session:${sessionId(input).toLowerCase()}`)
        return false;
