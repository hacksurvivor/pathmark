...
if (domain === "codex") {
...
else if (["audit", "doctor", "compact", "backup", "export", "import", "ingest", "purge"].includes(domain ?? "")) {
    ...
    console.log("Usage: pathmark [setup <client>|codex <command>|hook <event>|audit|doctor|compact|backup|export|import|ingest|purge]");
    console.log("");
    ...
    if (domain) {
        ...
        console.error("Usage: pathmark [setup <client>|codex <command>|hook <event>|audit|doctor|compact|backup|export|import|ingest|purge]");
        process.exitCode = 2;
    }
    else {
        await runMcpServer();
