...
export async function auditMemory(store, options = {}) {
    ...
    const referencedConsolidationEvidenceIds = new Set(evidenceReferences.filter((id) => consolidationEvidenceIds.has(id)));
    ...
    const references = recallEvents.flatMap((event) => event.activity?.type === "recall"
    ...
    const unprocessedEligibleEvidenceRecords = consolidationEvidence.filter((record) => !referencedConsolidationEvidenceIds.has(record.id) && !hasReviewedDisposition(record)).length;
    const eligibleEvidenceConclusionCoverage = ratio(referencedConsolidationEvidenceIds.size, consolidationEvidence.length);
    return {
        ...
        precision: feedback.labeledReferences > 0
            ? {
                status: "labeled",
            ...
            : {
                ...
                labelCoverage: 0,
                reason: "Recall activity records which ids were injected, but no user relevance labels exist. Use rate_recall or pathmark feedback to label exact recalled ids.",
            },
    };
}
function recallFeedback(selected, allById, selectedIds, windowStartMs) {
    ...
    for (const record of selected
        ...
        if (recall?.activity?.type !== "recall")
            continue;
        const recalledIds = new Set(recall.activity.memoryIds);
        for (const id of record.activity.relevantIds) {
            if (recalledIds.has(id) && selectedIds.has(id))
                labels.set(`${recall.id}\0${id}`, { recallId: recall.id, relevant: true });
        }
        for (const id of record.activity.irrelevantIds) {
            if (recalledIds.has(id) && selectedIds.has(id))
                labels.set(`${recall.id}\0${id}`, { recallId: recall.id, relevant: false });
        }
    }
