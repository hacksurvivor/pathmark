...
export async function prepareConsolidationBatch(store, options = {}) {
    ...
    const referencedIds = new Set(conclusions
...
export async function consolidateMemory(store, config, options = {}) {
...
export function consolidationNudge(batch, minimumEvidence) {
    if (batch.backlogCount < minimumEvidence || batch.evidence.length === 0)
        return "";
    return [
...
function evidenceSearchResult(record) {
...
function parseCandidates(answer, allowedIds, limit) {
    ...
    for (const value of values) {
        if (!isObject(value) || typeof value.text !== "string" || !value.text.trim())
            continue;
        if (isUnsafeMemoryText(value.text))
            continue;
        ...
        const evidenceIds = Array.isArray(value.evidenceIds)
            ? [...new Set(value.evidenceIds.filter((id) => typeof id === "string" && allowedIds.has(id)))]
            : [];
