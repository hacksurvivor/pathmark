...
const subpathSchema = z.string().min(1).max(2000).refine((value) => !path.isAbsolute(value) && !/[\\:\0]/.test(value) &&
    !value.split("/").some((part) => part === ".." || part === "" || part === ".") && /\.html?$/i.test(value), "Artifact subpath must be a relative HTML file without traversal");
...
async function allowedRoot(config, root) {
    if (!path.isAbsolute(root))
        throw new Error("artifactRoot must be an absolute directory");
    const canonical = await realpath(root);
    for (const allowed of config.artifactRoots ?? []) {
        try {
            if (inside(await realpath(allowed), canonical))
                return canonical;
        }
...
function redactValue(value, depth = 0) {
    if (depth > 20)
        throw new Error("Review selection is nested too deeply");
    if (typeof value === "string")
        return redactSecrets(value).text;
    if (Array.isArray(value))
        return value.map((item) => redactValue(item, depth + 1));
    if (value && typeof value === "object")
        return Object.fromEntries(Object.entries(value).map(([key, item]) => [redactSecrets(key).text, redactValue(item, depth + 1)]));
    return value;
}
export async function importScratchpadReview(store, config, input) {
    if (!input.tags.some((tag) => /^(project-id|project|workspace|namespace|session):.+/.test(tag)))
        throw new Error("A project or namespace scope is required");
    if (input.review.length > MAX_REVIEW_CHARS)
        throw new Error("Review exceeds 32000 characters");
    ...
    if (redactSecrets(parsed.artifact.subpath).redacted || redactSecrets(artifactRoot).redacted)
        throw new Error("Artifact paths must not contain secrets");
...
export async function checkArtifactReview(record, config) {
    if (!record.tags.includes(REVIEW_TAG) && record.source !== "scratchpad:review")
        return undefined;
    const result = { evidenceId: record.id, status: "unavailable", reason: "Invalid Scratchpad review evidence",
        checkedAt: new Date().toISOString(), authority: "reported-review-only" };
    let envelope;
    try {
        envelope = envelopeSchema.parse(JSON.parse(record.text));
    }
    catch {
        return result;
    }
    result.artifact = envelope.review.artifact;
    result.reportedDecision = envelope.review.decision;
    ...
    try {
        ...
        if (!inside(root, target))
            throw new Error("Artifact symlink escapes its bound directory");
        // Nonblocking avoids hanging on a substituted FIFO. O_NOFOLLOW rejects a final symlink swap.
        const file = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
        try {
            const before = await file.stat();
            if (!before.isFile() || before.size > MAX_HTML_BYTES)
                throw new Error("Artifact must be a regular HTML file of at most 2 MiB");
            ...
            if (total > MAX_HTML_BYTES || before.size !== total || before.mtimeMs !== after.mtimeMs || before.ctimeMs !== after.ctimeMs ||
                ...
                await realpath(path.resolve(root, envelope.review.artifact.subpath)) !== target)
                throw new Error("Artifact changed during verification; retry the check");
            const html = new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(bytes.subarray(0, total));
            if (html.includes("\0"))
                throw new Error("Artifact contains invalid NUL bytes");
        ...
        result.reason = code === "ENOENT" ? "Reviewed artifact is missing" : code ? `Artifact cannot be verified (${code})` : error.message;
    }
    return result;
}
