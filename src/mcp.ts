...
const SOFT_DELETE = { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false } as const;
...
export async function runMcpServer(): Promise<void> {
  ...
  server.registerTool(
    ...
    async ({ recallId, relevantIds, irrelevantIds, note }) =>
      jsonText(await recordRecallFeedback(store, config, { recallId, relevantIds, irrelevantIds, note })),
  );
  ...
  server.registerTool(
    ...
    async ({ days, evidenceLimit, cursor, maxProposals, tags, namespace, apply }) =>
      jsonText(
  ...
  server.registerTool(
    ...
    async ({ dedupe, dropDeleted, retentionDays, confirm }) =>
      jsonText(await store.compact({ dedupe, dropDeleted, retentionDays, dryRun: !confirm })),
  );
  ...
  server.registerTool(
    ...
    async ({ destination, tags, namespace, kind, includeDeleted, encrypted }) =>
      jsonText(await store.exportTo(destination, { tags, namespace, kind, includeDeleted, encrypted })),
  );
...
function scopedTags(tags: string[] | undefined, namespace: string | undefined): string[] {
