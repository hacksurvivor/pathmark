...
export async function taskBrief(store: PathmarkStore, config: PathmarkConfig, tags: string[], limit = 12) {
  if (!tags.length) throw new Error("A project or namespace scope is required");
...
export async function checkDecisions(store: PathmarkStore, config: PathmarkConfig, input: { tags: string[]; facts: PlanFacts; plan?: string; limit?: number }) {
  ...
  for (const item of brief.decisions) {
    const finding: DecisionFinding = { decisionId: item.id, revision: item.revision, text: item.text, rationale: item.decision.rationale,
    ...
    if (!record || recordRevision(record) !== item.revision || !isApprovedConclusion(record)) {
      finding.reasons.push("Decision changed during the check. Reload the current approved revision."); findings.push(finding); continue;
    }
...
function evaluate(rule: DecisionSpec["checks"][number], facts: PlanFacts): boolean | undefined {
  if (!Object.hasOwn(facts, rule.field)) return undefined;
  const value = facts[rule.field];
  if (rule.operator === "eq" || rule.operator === "neq") {
    if (typeof value !== typeof rule.value) return undefined;
    return rule.operator === "eq" ? value === rule.value : value !== rule.value;
  }
  if (rule.operator === "lte" || rule.operator === "gte") {
    if (typeof value !== "number" || typeof rule.value !== "number") return undefined;
    return rule.operator === "lte" ? value <= rule.value : value >= rule.value;
  }
  if (typeof value !== "string" || typeof rule.value !== "string") return undefined;
  return rule.operator === "includes" ? value.includes(rule.value) : !value.includes(rule.value);
}

export async function decisionOutcome(store: PathmarkStore, config: PathmarkConfig, input: { checkId: string; outcome: "useful" | "false-alarm" | "missed-conflict" | "decision-changed" | "no-effect"; note?: string }) {
  const check = await store.get(input.checkId);
  if (!check || !check.tags.includes("decision-check")) throw new Error("Decision check not found");
  const parsed = JSON.parse(check.text) as { findings: DecisionFinding[] };
...
export async function reviewQueue(store: PathmarkStore, tags: string[], limit = 3) {
  if (!tags.length) throw new Error("A project or namespace scope is required");
...
export async function reviewEvidence(store: PathmarkStore, input: { id: string; revision: string; status: EvidenceDisposition["status"]; reviewedBy: string; note?: string }) {
  const record = await store.get(input.id);
  if (!record || record.kind !== "memory" || record.activity) throw new Error("Raw evidence not found");
