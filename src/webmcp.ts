...
import type { PathmarkConfig, PathmarkRecordDraft } from "./types.js";
...
export async function runWebMcpCommand(args: string[]): Promise<void> {
  ...
  if (unknown.length > 0) throw new Error(`Unknown webmcp option: ${unknown[0]}`);
...
export async function startWebMcpServer(options: WebMcpServerOptions = {}): Promise<WebMcpServerHandle> {
  ...
  if (!address || typeof address === "string") throw new Error("Pathmark WebMCP server did not bind a TCP port");
  ...
  return {
    ...
    close: async () => {
      ...
      while (pendingActivityWrites.size > 0) await Promise.allSettled([...pendingActivityWrites]);
      await store.close();
    },
  };
}

async function trackActivityWrite(pending: Set<Promise<void>>, write: Promise<void>): Promise<void> {
  pending.add(write);
  try {
    await write;
  } finally {
...
async function recallFromBrowser(
  store: PathmarkStore,
  config: PathmarkConfig,
  call: BrowserCall,
  defaultTags: string[],
...
async function rateFromBrowser(
  store: PathmarkStore,
  config: PathmarkConfig,
  call: BrowserCall,
): Promise<Record<string, unknown>> {
...
async function recordWebMcpToolCall(
  store: PathmarkStore,
  config: PathmarkConfig,
  input: {
    toolName: string;
  ...
  const record: PathmarkRecordDraft = {
    id: randomUUID(),
    kind: "memory",
...
function requireSameOrigin(request: IncomingMessage, expectedOrigin: string, token: string): void {
  if (request.headers.origin !== expectedOrigin) throw new Error("forbidden_origin");
  if (request.headers["x-pathmark-webmcp-token"] !== token) throw new Error("invalid_token");
}
...
function parseBrowserCall(value: unknown): BrowserCall {
  ...
  if (!SESSION_PATTERN.test(sessionId)) throw new Error("invalid_session_id");
  if (!CALL_PATTERN.test(callId)) throw new Error("invalid_call_id");
  if (!isRecord(value.input)) throw new Error("invalid_input");
  return { sessionId, callId, input: value.input };
}

function requestTags(value: unknown, defaults: string[]): string[] {
...
function requiredString(value: unknown, name: string, limit: number): string {
  ...
  if (value.length > limit) throw new Error(`${name}_too_long`);
  return value.trim();
}

function optionalString(value: unknown, name: string, limit: number): string | undefined {
...
function optionalInteger(value: unknown, name: string, min: number, max: number): number | undefined {
  ...
  if (!Number.isInteger(value) || Number(value) < min || Number(value) > max) throw new Error(`${name}_invalid`);
  return Number(value);
}

function optionalKind(value: unknown): "memory" | "conclusion" | undefined {
  if (value === undefined || value === null || value === "") return undefined;
  if (value !== "memory" && value !== "conclusion") throw new Error("kind_invalid");
  return value;
}
...
function integerFlag(args: string[], name: string, fallback: number, min: number, max: number): number {
...
function endpointToolName(pathname: string): string {
  if (pathname === "/api/recall") return "pathmark_recall_memory";
  if (pathname === "/api/ask") return "pathmark_ask_memory";
  if (pathname === "/api/rate") return "pathmark_rate_recall";
...
function setSecurityHeaders(response: ServerResponse, nonce: string): void {
  ...
  response.setHeader(
    ...
    `default-src 'self'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'self'; img-src 'self' data:; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`,
  );
}
...
function webMcpPage(token: string, defaultTags: string[], nonce: string): string {
