...
export function selectRelevantResults(
  ...
  for (const candidate of ranked) {
    const durable =
      candidate.result.record.kind === "conclusion" ||
      candidate.result.record.tags.includes("decision") ||
      candidate.result.record.tags.includes("project-note");
    if (candidate.relevance < cutoff && !durable) continue;
    if (
      selected.some((existing) =>
        nearDuplicate(existing.terms, candidate.terms, existing.result.record.text, candidate.result.record.text),
      )
    ) {
      continue;
    }
...
export function selectRelevantResultsByIntent(
  ...
  if (intents.length <= 1) return selectRelevantResults(results, query, limit, options);
  ...
  for (const intent of intents) {
    for (const result of selectRelevantResults(results, intent, perIntentLimit, intentOptions)) {
      if (selectedIds.has(result.record.id)) continue;
...
export function splitQueryIntents(query: string): string[] {
  ...
  for (const piece of pieces) {
    ...
    if (key && !distinct.has(key)) distinct.set(key, piece);
  }
  return distinct.size > 1 ? [...distinct.values()] : [query];
}

export function filterLowSignalResults(results: SearchResult[]): SearchResult[] {
...
function isLowSignalCapture(result: SearchResult): boolean {
  ...
  if (!result.record.tags.includes("role-assistant")) return false;
  return /^(?:i(?:['’]m| am)\s+(?:starting|checking|verifying|running|opening|reviewing|tracing|testing)|i['’]ll\s+(?:check|verify|run|inspect|open|review|trace|test|start|audit|compare|update|fix))\b/i.test(
    text,
  );
}
...
function nearDuplicate(left: Set<string>, right: Set<string>, leftText: string, rightText: string): boolean {
  ...
  if (intersection / Math.min(left.size, right.size) >= NEAR_DUPLICATE_CONTAINMENT) return true;
  ...
  if (shortestLength < 400) return false;
  let commonPrefix = 0;
