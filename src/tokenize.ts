...
export function tokenizeSearchText(text: string): string[] {
  ...
  for (const match of normalized.matchAll(WORD_RE)) {
    ...
    for (const part of match[0].split(/[-']+/)) addTerm(terms, part);
  }
...
export function isCjkSearchTerm(term: string): boolean {
...
function addTerm(terms: Set<string>, value: string): void {
  ...
  if (!term) return;
  if ([...term].length > 1 || isCjkSearchTerm(term)) terms.add(term);
}
