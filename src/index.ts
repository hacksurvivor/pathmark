...
if (domain === "codex") {
...
} else if (["project", "review", "decision", "chat", "feedback", "consolidate", "audit", "doctor", "compact", "backup", "export", "import", "ingest", "purge"].includes(domain ?? "")) {
  ...
  console.log("Usage: pathmark [setup <client>|codex <command>|hook <event>|project|review|decision|chat|feedback|consolidate|audit|doctor|compact|backup|export|import|ingest|purge]");
  console.log("");
  console.log("No arguments starts the Pathmark MCP stdio server.");
} else {
  if (domain) {
    console.error(`Unknown command: ${domain}`);
    console.error("Usage: pathmark [setup <client>|codex <command>|hook <event>|project|review|decision|chat|feedback|consolidate|audit|doctor|compact|backup|export|import|ingest|purge]");
    process.exitCode = 2;
  } else {
    await runMcpServer();
  }
