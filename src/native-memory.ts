...
import { namespaceTag, type PathmarkStore } from "./store.js";
...
export async function readClaudeCodeMemories(root: string, projectFilter?: string): Promise<NativeMemoryFile[]> {
  const projects = await readdir(root, { withFileTypes: true }).catch(() => []);
  const memories: NativeMemoryFile[] = [];
  for (const project of projects) {
    if (!project.isDirectory()) continue;
    if (projectFilter && !project.name.toLowerCase().includes(projectFilter.toLowerCase())) continue;
    const projectDir = path.join(root, project.name);
    const memoryDir = path.join(projectDir, "memory");
    const files = (await readdir(memoryDir).catch(() => [] as string[])).filter(
...
export function parseMemoryMarkdown(raw: string, fallbackName: string): { name: string; type?: string; text: string } {
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/.exec(raw);
  const frontmatter = match ? frontmatterFields(match[1]) : new Map<string, string>();
...
export async function resolveSlugPath(slug: string, base = path.parse(process.cwd()).root): Promise<string | undefined> {
  return walkSlug(slug, base, 0);
}

async function walkSlug(remaining: string, dir: string, depth: number): Promise<string | undefined> {
  if (!remaining) return dir;
  if (depth >= MAX_SLUG_DEPTH || !remaining.startsWith("-")) return undefined;
...
export function nativeMemoryDraft(memory: NativeMemoryFile, options: { namespace?: string; redact: boolean }): PathmarkRecordDraft {
  const tags = [CLAUDE_CODE_MEMORY_TAG];
  if (memory.type) tags.push(`memory-type:${memory.type.toLowerCase()}`);
  ...
  if (options.namespace) tags.push(namespaceTag(options.namespace));
...
export async function importClaudeCodeMemories(
  store: PathmarkStore,
  memories: NativeMemoryFile[],
  options: { namespace?: string; redact: boolean; dryRun: boolean },
  ...
  const additions: PathmarkRecordDraft[] = [];
  for (const memory of memories) {
    ...
    result.updated += 1;
    if (!options.dryRun) await store.update(memory.id, { text: draft.text, tags: draft.tags, source: draft.source });
  }
  if (!options.dryRun && additions.length) await store.addRecords(additions);
  return result;
}

// Compare the way the store persists tags (trimmed, lowercased, deduped); ignore the quarantine
// tag the store adds on its own.
function sameTags(stored: string[], draft: string[]): boolean {
  const normalize = (tags: string[]) =>
    [...new Set(tags.map((tag) => tag.trim().toLowerCase()).filter((tag) => tag && tag !== QUARANTINED_MEMORY_TAG))]
      .sort()
      .join("\n");
