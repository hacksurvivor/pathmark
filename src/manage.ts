...
export async function runManagementCommand(command: string, args: string[]): Promise<void> {
  ...
  if (command === "review" || command === "decision") {
    ...
    if (command === "review" && action === "queue") result = await reviewQueue(store, tags, numberOption(options, "limit", 3));
    ...
    } else if (command === "decision" && action === "brief") result = await taskBrief(store, config, tags);
      ...
      for (const id of input.evidenceIds) {
        ...
        if (!evidence || evidence.kind !== "memory" || !tags.every((tag) => evidence.tags.includes(tag))) throw new Error("Decision evidence must exist in the selected scope");
      }
  ...
  if (command === "feedback") {
    ...
    if (!recallId) throw new Error(`Feedback requires --recall-id=ID.\n${USAGE}`);
    console.log(
      JSON.stringify(
...
function importedApproval(value: unknown): PathmarkApproval | undefined {
  ...
  return {
    ...
    ...(isObject(value.evidenceRevisions) ? { evidenceRevisions: value.evidenceRevisions as Record<string, string> } : {}),
  };
}
